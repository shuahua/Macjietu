...
final class SettingsWindowController: NSObject, NSWindowDelegate {
    ...
    private let exportScaleOptions: [(title: String, value: Double)] = [
        ("标准 1x", 1),
        ("高清 2x", 2),
        ("超清 3x", 3),
        ("极清 4x", 4),
    ...
    func show() {
        ...
        let qualityLabel = NSTextField(labelWithString: "保存清晰度")
        ...
        exportScalePopup.frame = CGRect(x: 106, y: 50, width: 150, height: 28)
        let selectedIndex = exportScaleOptions.firstIndex { $0.value == settings.exportScale } ?? 1
    ...
    private func saveCurrentSettings() {
        let settings = AppSettings(
            ...
            saveDirectory: nil,
            exportScale: exportScaleOptions[max(0, exportScalePopup.indexOfSelectedItem)].value
        )
